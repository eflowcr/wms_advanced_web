 ...
 * hover y foco con la misma utilidad. `pr-12` deja lugar a la pista o a la ×.
 */
export function searchFieldClasses(disabled: boolean): string {
  const base =
    'h-full w-full min-w-0 rounded-l-full border border-r-0 border-solid pl-4 pr-12 outline-none ' +
...
export function searchButtonClasses(disabled: boolean): string {
  const base =
  ...
  return disabled
    ? base + 'border-default text-disabled cursor-not-allowed'
